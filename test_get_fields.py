...
def main():
    ...
    print(f"Updated player: {player.name}, ID: {player.id}, Status: {my_pb2.Player.Status.Name(player.status)}")
    print()
    ...
    player2 = my_pb2.Player(id=2, name="Player Two", status=my_pb2.Player.Status.CONFIRMED)
    player1.MergeFrom(player2)
    print(f"Merged player: ID={player1.id}, Name={player1.name}, Status={my_pb2.Player.Status.Name(player1.status)}")
    ...
    print(f"Current status value {player.status} = {my_pb2.Player.Status.Name(player.status)}")
    print()
