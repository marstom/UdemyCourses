...
class BackpackManager(my_pb2_grpc.BackpackManagerServicer):
    ...
    def pack(self, request, context):
        ...
        print(request.item)
        Timestamp
        return my_pb2.BackpackItemResponse(
            item=request.item, msg_time=datetime.datetime.now()
        )
    ...
    def unpack(self, request, context):
        ...
        print("00000000")
        return my_pb2.BackpackItemResponse(
            item=self.items.pop(), msg_time=datetime.datetime.now()
        )
    ...
    def pack_and_immediately_show_id(self, request_iter, context):
        ...
        for request in request_iter:
            ...
            self.items.append(request.item_name)
            time.sleep(0.5)
            yield my_pb2.PackResponseWitId(
                added=f"Item {request.item_name} packed", id=len(self.items)
            )
    ...
    def unpack_and_immediately_show(self, request_iter, context):
        ...
        for request in request_iter:
            ...
            if len(self.items) >= request.item_idx:
                ...
                yield my_pb2.UnpackContinouslyResponse(
                    fetched_item=f"Item fetched: {fet_item}"
                )
            else:
